...
const useModalKeyboard = (
  ...
  const handleArrowKeys = (e: KeyboardEvent) => {
    ...
    const footerElement = modalRef.current?.querySelector<HTMLElement>(
    ...
    const targetElement =
      e.key === "ArrowLeft"
        ? focusableFooterElements.at(-1)
        : focusableFooterElements[0];

    if (targetElement) {
      focusState.current.currentIndex =
        focusState.current.elements.indexOf(targetElement);
      targetElement.focus();
    }
  };
  ...
  const handleEnterKey = (e: KeyboardEvent) => {
    ...
    if (footerButtons.length === 1 && footerButtons[0].tagName === "BUTTON") {
      ...
      const currentIndex = focusState.current.elements.indexOf(
        footerButtons[0]
      );
